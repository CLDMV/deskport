#!/usr/bin/env node
/**
 * Remote Dev Launcher — trigger.
 *
 * Run this ON THE REMOTE host (typically from an npm/pnpm script). It writes a
 * trigger file that the local "Remote Dev Launcher" VSCode extension watches.
 ...
 *
 *   node .devlauncher/trigger.mjs <target-name>
 *
 * Example package.json scripts:
 *   "launchLocal1": "node .devlauncher/trigger.mjs app",
 *   "launchLocal2": "node .devlauncher/trigger.mjs admin"
 *
 * The target names must match keys under "targets" in .devlauncher/config.json.
...
if (!target) {
	process.stderr.write("trigger: usage - node .devlauncher/trigger.mjs <target-name>\n");
	process.exit(1);
}
...
process.stdout.write(
	"Remote Dev Launcher: requested target " +
