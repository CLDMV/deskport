...
A minimal Electron app for testing the **DeskPort** extension. It
opens one window and shows when it loaded.

## Use it as a test target

This folder ships a `.deskport/config.json` with a single `app` target
...
2. Make sure the DeskPort extension is installed on your local
   machine (`pnpm run install-local` from the repo root, run on that machine).
3. Click **🚀 DeskPort** in the status bar and pick **Example Electron App**.

When the workspace is remote, the extension mirrors this folder into the local
clone directory (see the `deskport.clonePath` setting) and runs
